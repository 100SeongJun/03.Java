...
public class Server {
	...
	static void startServer() {
		...
		try {
			...
			serverSocket.bind(new InetSocketAddress("192.168.20.8", 8888));
		} catch (Exception e) {
			...
			public void run() {
				...
				while (true) {
					try {
						...
						System.out.println("[연결을 수락합니다 : " + socket.getRemoteSocketAddress() + ": "
				...
				public void run() {
					try {
						...
						try {
							...
							System.out.println("[클라이언트 통신 불가능 : " + socket.getRemoteSocketAddress() + ": "
									+ Thread.currentThread().getName() + "]");
							socket.close();
			...
			Runnable runnable = new Runnable() {
				...
				public void run() {
					try {
						...
						try {
							System.out.println("[클라이언트 통신 불가눙 : " + socket.getRemoteSocketAddress() + ": "
									+ Thread.currentThread().getName() + "]");
							connections.remove(Client.this);
							socket.close();
